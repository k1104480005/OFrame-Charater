...
New motions SHALL default to a single direction (usually right-facing or front-facing), so only one generated direction exists.

#### Scenario: Default single-direction motion
- **WHEN** the user creates a new motion without choosing a direction strategy
- **THEN** the motion contains only the single default direction
...
When the user chooses 8 directions, the system SHALL issue 4 generation calls and derive the other 4 directions by horizontal mirroring: right mirrors to left, up-right mirrors to up-left, and down-right mirrors to down-left, while up and down are generated.

#### Scenario: 8-direction configuration
- **WHEN** the user chooses the 8-direction strategy for a motion
- **THEN** the system generates 4 basic directions and derives the remaining 4 directions by horizontal mirroring
